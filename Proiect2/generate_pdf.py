...
def gen_output(target, output, imresize_file, imresize_fn, args):
    subprocess.check_call(
            ['python3', 'main.py', target, output, *args])
...
def add_task_1_5(c, tmp_dir, data_dir):
    ...
    def random_path_algorithm():
        return random.choice(
                ['random', 'greedy', 'dynamicprogramming'])
    ...
    def process_fn(keyword, target, output, imresize_file, desc_file):
        ...
        with open(desc_file, 'w+t') as f:
            f.write('{} from google, with {} lines, {} columns and path algorithm {}'.format(keyword, del_lines, del_columns, path_algorithm))

        gen_output(target, output, imresize_file, args=args, imresize_fn=delta_imresize(del_lines, del_columns))
