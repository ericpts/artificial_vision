class Parameters(object):
    def __init__(self,
                ...
                init_output_width: int, init_output_height: int,
...
# Resize the image to fit the blocks exactly.
        (self.output_width, self.output_height) = (
                (self.blocks_per_height - 1) * (self.block_height - self.overlap_height) + self.block_height,
                (self.blocks_per_width - 1) * (self.block_width - self.overlap_width) + self.block_width)
    ...
    def get_blocks_per_dimension(self):
        (blocks_per_height, blocks_per_width) = (
            int(1 + (self.init_output_height - self.block_height) // (self.block_height - self.overlap_height)),
            int(1 + (self.init_output_width - self.block_width) // (self.block_width - self.overlap_width))
            )
...
# The last blocks should fit completely within the image.
        assert (blocks_per_height - 1) * (self.block_height - self.overlap_height) + self.block_height <= self.init_output_height
        assert (blocks_per_width - 1) * (self.block_width - self.overlap_width) + self.block_width <= self.init_output_width

# We should not be able to add any more blocks.
        assert (blocks_per_height - 0) * (self.block_height - self.overlap_height) + self.block_height > self.init_output_height
        assert (blocks_per_width - 0) * (self.block_width - self.overlap_width) + self.block_width > self.init_output_width
