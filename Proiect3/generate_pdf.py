...
def gen_from_sample(sample: Path, tmp_dir: Path):
  for algo in algorithms:
    ...
    subprocess.check_call(
        [
            'python3', 'main.py', '--sample', sample, '--output',
            str(out), '--overlap',
            str(0.16), '--texture-block-size', *['50', '50'], '--output-size', *['500', '500'],
            '--algorithm', algo, '--texture-block-count',
            str(10000)
        ],
...
def gen_textured(texture: Path, out: Path, target: Path):
  subprocess.check_call(
      [
          ...
          str(texture), '--output',
          str(out), '--overlap',
          str(0.16), '--texture-block-size', *['50', '50'], '--texture-block-count',
          str(1000), '--algorithm', 'texture-transfer', '--transfer-image',
          str(target), '--transfer-niterations',
          str(5)
      ],
      stdout=subprocess.DEVNULL)
...
def add_task_1_1(executor: concurrent.futures.Executor, tmp_dir: Path, data_dir: Path):
  ...
  for f in glob.glob(str(data_dir / 'img*.png')):
    ...
    add_todo(sample, '{} original'.format(sample.stem))
...
def add_task_1_2(executor: concurrent.futures.Executor, tmp_dir: Path, data_dir: Path):
  ...
  def add_google_images():
    keywords = ['florin salam', 'mark zuckerberg', 'mr robot']
