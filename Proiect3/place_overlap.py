...
def place_overlap(params: Parameters, sample_img: ndarray) -> ndarray:
    ...
    def place_piece_on_output(output: ndarray, i: int, j: int, piece: int):
        ...
        (end_height, end_width) = (start_height + params.block_height,
    ...
    def cost_horizontal(i: int, j: int, piece: int) -> float:
        ...
        if j == 0:
            # This is the first piece in its' row, so there is no neighbour to the
            # left. Therefore, the cost is 0.
            return 0

        return distance(
            output[start_height:start_height + params.block_height, start_width:
                   start_width + overlap_width],
            # Take all lines of the block, but only columns 0 .. overlap_width - 1
    ...
    def cost_vertical(i: int, j: int, piece: int) -> float:
        ...
        return distance(
            output[start_height:start_height + overlap_height, start_width:
                   start_width + params.block_width],
    ...
    def cost_overlap_vertical_horizontal(i: int, j: int, piece: int) -> float:
        ...
        return distance(
            output[start_height:start_height + overlap_height, start_width:
                   start_width + overlap_width],
            # Take only the bottom 0..overlap_height - 1 rows, and all columns inside
            # them (chop off the bottom side).
    ...
    def get_best_fit(i: int, j: int) -> int:
        ...
        def cost(piece: int) -> float:
            ret = cost_horizontal(i, j, piece) + cost_vertical(
                i, j, piece) - cost_overlap_vertical_horizontal(i, j, piece)
            return ret
