...
def get_column_path_dynamicprogramming(energy):
  ...
  for i in range(1, n):
    ...
    best[i] = energy[i] + np.minimum(best[i - 1], np.minimum(shift_left, shift_right))
...
def get_google_images(keywords: List[str], outputs: List[Path], MAX_LINKS=100, overwrite=False):
  if len(keywords) != len(outputs):
    ...
    raise ValueError('Received more keywords than there are outputs: {} != {}'.format(
        len(keywords, len(outputs))))
  ...
  for (keyword, output) in zip(keywords, outputs):
    ...
    links = google_images_download.get_image_links(
        search_keywords=[keyword], keywords=['high resolution'], requests_delay=0, limit=MAX_LINKS)
...
def image_energy(img):
  ...
  return np.sqrt(sobel_h(grayscale)**2 + sobel_v(grayscale)**2) / np.sqrt(2)
